...
# -*- coding: utf-8 -*-
"""
IBKR Gateway 多品种 H1 技术位置雷达 + 企业微信预警
==============================================
...
用法：
    ...
    # 批量运行默认品种
    python xauusd_technical_analysis_chart_v2.py --batch

    # 不带品种时读取同目录配置文件 xauusd_technical_analysis_chart_v2.json
    python xauusd_technical_analysis_chart_v2.py

    # 计划任务：只要设置 WEWORK_WEBHOOK_URL，位置状态变化时自动推送
    python xauusd_technical_analysis_chart_v2.py GC ES NQ YM
...
def compute_profile_stats(df: pd.DataFrame, bins: int = PROFILE_BINS) -> Dict[str, Any]:
    ...
    if not np.any(counts > 0):
        return {
            ...
            "HVN": [], "LVN": [], "counts": counts, "edges": edges, "centers": centers,
        }
...
def draw_right_profile(
    ...
    for y in profile.get("HVN", []) or []:
        ax.hlines(float(y), x_start, x_end, color="#8bd3ff", linewidth=1.0, alpha=0.95, zorder=3)
    for y in profile.get("LVN", []) or []:
        ax.hlines(float(y), x_start, x_end, color="#c9a7ff", linewidth=0.8,
                  linestyle=":", alpha=0.9, zorder=3)

    ax.text(x_start, ax.get_ylim()[1], f"{profile['Mode']}  POC", color="#ffd166", fontsize=7.5,
...
def draw_chart(
    ...
    profile = compute_profile_stats(df_plot)
    ...
    hvn_text = ", ".join(fmt_price(float(v), dec) for v in profile.get("HVN", []) or []) or "N/A"
    lvn_text = ", ".join(fmt_price(float(v), dec) for v in profile.get("LVN", []) or []) or "N/A"
    ...
    info_text = (
        ...
        f"E20Dst: {e20_text}\n"
        f"RVOL20: {rvol_text}\n"
        f"Setup : {location['setup_status']}\n"
        f"Score : {location['location_score']}/100\n"
        f"Dir   : {location['candidate_direction']}\n"
        f"AI Req: {location['analysis_required']}\n\n"
